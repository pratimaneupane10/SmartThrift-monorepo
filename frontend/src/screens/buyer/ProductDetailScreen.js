...
export default function ProductDetailScreen({ route, navigation }) {
  ...
  async function handleShare() {
    await Share.share({ message: `Check out ${item.title} on Smart Thrift for NPR ${item.price}!` });
  }
        ...
        <View style={{ padding: spacing.lg }}>
          <View style={styles.badgeRow}>
            <View style={styles.verifiedBadge}>
              <Text style={{ color: '#FFFFFF', fontSize: 10, fontWeight: '700' }}>✓ PREMIUM SELLER</Text>
            </View>
            {item.demand === 'high' && (
              <View style={styles.demandBadge}>
                <Text style={{ color: '#FFFFFF', fontSize: 10, fontWeight: '700' }}>HIGH DEMAND</Text>
              </View>
            )}
          </View>

          <Text style={[typography.heading, { marginTop: spacing.sm, fontSize: 22 }]}>{item.title}</Text>
          ...
          <Pressable style={styles.sellerRow} onPress={() => navigation.navigate('SellerProfile')}>
            <View style={styles.sellerAvatar} />
            <View style={{ flex: 1, marginLeft: spacing.sm }}>
              <Text style={typography.subheading}>Pradip kc</Text>
              <Text style={[typography.caption, { color: colors.primaryTeal }]}>★ 4.8 · View Store</Text>
            </View>
            <Text style={[typography.caption, { color: colors.primaryTeal }]}>View Store →</Text>
          </Pressable>
          ...
          <View style={styles.detailsGrid}>
            <View style={styles.detailBox}>
              <Text style={typography.caption}>MATERIAL</Text>
              <Text style={[typography.body, { fontWeight: '700' }]}>100% Virgin Wool</Text>
            </View>
            <View style={styles.detailBox}>
            ...
            <View style={styles.detailBox}>
              ...
              <Text style={[typography.body, { fontWeight: '700' }]}>Express Worldwide</Text>
            </View>
            <View style={styles.detailBox}>
...
const styles = StyleSheet.create({
  topBar: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: spacing.md, borderBottomWidth: 1, borderBottomColor: colors.border },
  ...
  contactBtn: { flex: 1, backgroundColor: colors.primary, borderRadius: radius.md, padding: spacing.md, alignItems: 'center' },
